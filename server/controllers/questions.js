...
export const editQuestion = async (req, res) => {
  const { id: _id } = req.params;
  const { questionTitle, questionBody, codeBlock, } = req.body;

  if (!mongoose.Types.ObjectId.isValid(_id)) {
    return res.status(404).send("Question unavailable...");
  }

  const updatedQuestion = { questionTitle, questionBody, codeBlock };

  await Questions.findByIdAndUpdate(_id, updatedQuestion, { new: true });
